...
void suPHP::API_Linux_Logger::log(const std::string& classification,
  ...
  // Construct logline
  logline =
      std::string(timestr) + " [" + classification + "] " + message + "\n";
...
void suPHP::API_Linux_Logger::init(const Configuration& config) throw(
  ...
  this->logFd =
      ::open(config.getLogfile().c_str(),
             O_WRONLY | O_CREAT | O_APPEND | O_NOCTTY, S_IRUSR | S_IWUSR);

  // Check wheter something failed
  if (this->logFd == -1) {
    throw IOException("Could not open logfile " + config.getLogfile(), __FILE__,
                      __LINE__);
  }
  ...
  if (::fcntl(this->logFd, F_SETFD, FD_CLOEXEC)) {
    ...
    throw IOException("Could not set close-on-exec flag on logfile", __FILE__,
                      __LINE__);
  }
...
bool suPHP::API_Linux_Logger::isInitialized() { return this->ready; }

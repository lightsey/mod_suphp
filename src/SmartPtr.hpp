...
suPHP::SmartPtr<T>::SmartPtr(const SmartPtr<T>& ref) {
  ...
  this->ptr = ref.ptr;
  if (ref.ptr != NULL) this->increment(ref.ptr);
}

template <class T>
suPHP::SmartPtr<T>::~SmartPtr() {
  if (this->ptr != NULL) this->decrement(this->ptr);
...
template <class T>
const SmartPtr<T>& suPHP::SmartPtr<T>::operator=(const SmartPtr<T>& ref) {
  this->reset(ref.ptr);
  return *this;
}
...
T& suPHP::SmartPtr<T>::operator*() const throw(PointerException) {
  if (this->ptr == NULL)
    throw PointerException("Cannot dereference NULL pointer", __FILE__,
                           __LINE__);
  return *(this->ptr);
}
...
T* suPHP::SmartPtr<T>::operator->() const throw(PointerException) {
  if (this->ptr == NULL)
    throw PointerException("Cannot access member of NULL pointer", __FILE__,
                           __LINE__);
  return this->ptr;
}
...
T* suPHP::SmartPtr<T>::release() throw(PointerException) {
  ...
  if (c > 1) {
    throw PointerException(
        "Cannot release object hold by more than one SmartPointer.", __FILE__,
        __LINE__);
  } else {
...
void suPHP::SmartPtr<T>::increment(T* key_ptr) {
  ...
  if (SmartPtr<T>::counter->find(key_ptr) != SmartPtr<T>::counter->end()) {
